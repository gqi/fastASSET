...
#' @param max_numtraits_per_side Maximum number of traits per side for subset search. Default to be 16. The program reports an error if \code{>max_numtraits_per_side} pass pre-screening and the adjusted z-statistics have the same sign. Such SNPs take significant computational time. Increasing this number allows the program to include some of those SNPs.
#'
